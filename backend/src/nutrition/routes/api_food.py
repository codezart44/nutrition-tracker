...
@blueprint_food.post("/search")
def get_foods_search():
    payload = request.get_json() or {}
    pattern = validate_str(payload.get("pattern"))
    
    with get_connection(current_app.config["DB_PATH"]) as con:
        result = select_food_description_like(con, pattern)
